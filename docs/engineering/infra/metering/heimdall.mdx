---
title: heimdall (per-pod billing agent)
description: How we collect per-millisecond CPU, memory, and disk metrics for customer deployment billing.
---

## What this is

Heimdall is the agent that reads kernel counters off every node hosting customer pods and writes them to ClickHouse, where billing is computed at query time. It runs as a DaemonSet on the `untrusted` nodepool and writes one row per container per tick (plus extras when containers start or stop).

Two things to keep in mind for the rest of this doc:

> **Never overcharge a customer. Undercharging is acceptable.**

Every design choice flows from that rule.
...
The program skips the 14-byte Ethernet header (tc on veth sees L2-framed skbs), branches on `skb->protocol`, reads the remote IP (`iph->daddr` on pod-egress, `iph->saddr` on pod-ingress), classifies it via the same RFC1918 check the old cgroup_skb collector used, and `__sync_fetch_and_add`s `skb->len` into the right slot.

The same idempotent counter math used for CPU works here: `max(network_egress_public_bytes) - min(...)` over a billing window is replay-safe and unaffected by extra readings. Two heimdall pods overlapping during a rolling update both see the same monotone counter value, write distinct rows, and the billing query stays correct.
...
Concretely: a bug in this package can cause under-billing of network bytes, or crash heimdall itself (which is isolated on the node). It cannot drop, corrupt, or reorder customer packets, and it cannot break customer pod networking.
...
Note that all resource columns are `Int64`, not `UInt64`. A forgotten clamp in a billing query on a UInt64 column would underflow to ~1.8 × 10^19 and bill the customer for 584 thousand CPU-years. Int64 underflow shows up as a visible negative number, which fails loud instead of silently corrupting an invoice.
...
A container ran for exactly one hour at exactly one vCPU. We do not know how often heimdall sampled it. The bill is the same in all three cases.

| Sampling cadence | Checkpoints written | `max - min` | Bill |
|------------------|---------------------|-------------|------|
...
* **Memory integration accuracy.** Memory is a gauge, not a counter, so we have to integrate. The ClickHouse MV computes `(sum(memory_bytes) / sample_count) * window_ms` per day. More samples means the mean is closer to the true integral. Fewer samples means the same shape, blurrier.
* **Worst case undercharge on container exit.** If the container dies between the last periodic tick and CRI catching the exit event, we miss whatever CPU it used in that gap. Bounded by the tick interval (currently 5 seconds).
...
`container_uid = pod_uid + restart_count` is the key. It is the unique billing entity for a container's lifetime. When kubelet restarts a container, restart_count increments and the new container is a new billing window from t=0.
...
instance_checkpoints_v1                ← raw, ReplacingMergeTree, 95 day TTL
    ...
    │
    └─→ container_usage_per_day_v1        ← billing grain, 400 day TTL
            │
            └─→ workspace_usage_per_month_v1   ← invoice grain, no TTL
```
...
The dashboard MVs and the billing MVs serve different needs and we keep them strictly separate. Dashboard MVs are explicitly marked "NEVER FOR BILLING" because their `*_sum` columns can double-count under ReplacingMergeTree dedup races (MVs fire on inserts, not on merges). Billing reads the raw table through `FINAL`, which is slower but correct.
...
CPU seconds per container per period:

```sql
(max(cpu_usage_usec) - min(cpu_usage_usec)) / 1e6
```

Memory byte-seconds (time-weighted, pre-computed by the daily MV):

```sql
sum(memory_bytes) / sample_count * (max_ts - min_ts)
```

Disk byte-seconds (allocation is constant per container today, so this is just `size × duration`):

```sql
max(disk_allocated_bytes) * (max(ts) - min(ts))
```

Network (column reserved, not yet populated):
...
| Failure | What happens | Why it does not overcharge |
|---------|--------------|----------------------------|
| Heimdall pod crashes | In-flight buffer is lost (a few seconds of data). Next checkpoint after restart sets a fresh `min` for that window. | Kernel counter does not reset. The lost interval is undercharged, never overcharged. |
| Node dies | Same. | Same. |
...
| CRI TaskExit event dropped | Last periodic tick is treated as the final reading. | Bounded undercharge of one collection interval (currently 5 seconds). |
...
| Heimdall not running on a node | Everything on that node is unbilled. | Code cannot detect absence; mitigation is alerting at ingestion (no checkpoints from a known node for N minutes). |
...
Manifest at `dev/k8s/manifests/heimdall.yaml`; production equivalents live in the infra repo.
...
| `pkg/clickhouse/schema/029_container_usage_per_day_v1.sql` | Billing grain |
| `pkg/clickhouse/schema/031_workspace_usage_per_month_v1.sql` | Invoice grain |
