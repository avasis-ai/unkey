...
5. To bill, we use the exact same math but with a month-long window. One number, one invoice line.
...
### Step 5. Chart and billing query
...
To bill, we use the same `max - min` math over the billing window (one month, say) instead of 15 seconds:
...
One query shape. Different window sizes.
...
Billing queries don't need the fix because they always end in the past.
...
### 1. Duplicate writes would inflate the bill
...
If we later add a new aggregation (e.g. per-15-minute billing for a specific customer), the snapshot model is fine: write a new query, done. The interval-row model needs to re-derive every row from raw data, which we might not have anymore.
...
4. **Billing can pick either.** Currently we bill allocated for CPU/memory (customer reserved it, so they pay for it) and used for network/disk. Schema is agnostic.
...
## Billing accuracy: where failures land

The never-overcharge rule propagates through every stage. Every failure mode is either (a) idempotent (a duplicate doesn't change the bill) or (b) fails open (missing data only undercounts). Nothing can inflate a bill.

| Failure | What happens | Billing effect |
|---|---|---|
| Heimdall crashes mid-tick | Next tick starts fresh; no local state was lost because we don't keep any. | Undercharge by at most 5 seconds |
| In-memory batch buffer lost (heimdall OOM) | A few seconds of rows weren't flushed. | Undercharge by at most the buffer-flush interval |
...
| Both CRI and informer miss a lifecycle event | Last 5-second periodic tick is our best data. | Undercharge by at most 5 seconds of CPU on exit |
| Rollup refresh lagged | Chart uses the raw table for the recent tip. | No effect on billing; chart shows a narrow bucket for a moment |
...
The one failure mode the design doesn't handle well: a **wrong allocation value** sampled from a stale pod-spec cache. Could overbill if the stale value happened to be high. Mitigation: the informer cache is eventually consistent with the K8s API, and any staleness is consistent across the whole platform, not just heimdall.
...
<AccordionGroup>
  <Accordion title="Why don't we just write one row per 15 seconds saying 'container X used Y CPU'?">
    Because a duplicate row inflates the bill. See the "Why not just write..." section above, or the full defence in [why-counters-not-deltas](./why-counters-not-deltas).
  </Accordion>
  ...
  <Accordion title="Could `max - min` ever go negative and overcharge someone?">
    All counter columns are `Int64`, not `UInt64`. A negative result would show up as a visible negative number in the invoice pipeline and fail a downstream sanity check. Contrast with `UInt64`: underflow would wrap to roughly 1.8 × 10^19 and silently bill 584 thousand CPU-years. The sign bit is deliberate insurance.
  </Accordion>

  <Accordion title="Why not use Prometheus or cAdvisor or OpenTelemetry?">
    cAdvisor's per-container metrics broke for gVisor pods in 2021 and never fully recovered ([gvisor#6500](https://github.com/google/gvisor/issues/6500)). Prometheus would work for scraping but adds a scrape pipeline and a second source of truth; heimdall is already reading the same files, so it writes straight to the billing table. OpenTelemetry's SDK is for application instrumentation, not sampling kernel counters. Shortest path wins.
  </Accordion>
</AccordionGroup>
