...
description: The defence for storing raw cumulative counters and computing billing with max minus min at query time. Citations and prior art for pushing back on the "just write deltas" proposal.
---
...
The short version: **cumulative counter rows are idempotent under duplicate writes. Delta rows are not.** In any pipeline with retries, rolling updates, or backfills, duplicate rows will happen. With cumulative counters they don't affect the bill. With deltas they do.

The rest of this doc is why that difference shapes the entire architecture.

## The invariant we protect

> Never overbill a customer. Underbilling is acceptable.

Underbilling shows up as less revenue. Overbilling shows up as refund requests, credit notes, audit trails, and occasionally lawsuits. The asymmetry forces an asymmetric design: the pipeline must be *structurally* incapable of double-counting, even when individual components misbehave.
...
In all six scenarios, the same rows land in ClickHouse more than once. The question is whether that inflates the bill.

## Delta model: duplicates inflate the bill
...
`max(counter) - min(counter) = 2100 - 1500 = 600`. **The duplicate row has zero effect on the bill.** `max` and `min` over the same set of values are identical whether you include a value once or a hundred times.
...
Every serious monitoring system that cares about billing-grade correctness converges on cumulative counters:
...
2. We had a separate, independent audit path that re-reads raw counters and cross-checks the billed number, so a delta-pipeline bug would be caught before invoicing goes out.
...
- [Stripe's "Online Migrations at Scale"](https://stripe.com/blog/online-migrations). Not metering-specific but illustrates the same "make the pipeline idempotent at the data layer" principle.
