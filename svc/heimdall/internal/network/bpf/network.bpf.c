...
//
// 4096 entries comfortably covers any realistic node's pod count (default
// kubelet --max-pods is 110 on EKS; one veth per pod).
struct counters {
    __u64 egress_public;
...
// is_v4_private returns 1 if addr is in RFC1918, link-local, CGNAT, or
// loopback space. `addr` is __be32 straight out of the IP header; reading
// it byte-by-byte gives a.b.c.d directly on any endianness. All cluster
// CIDRs sit under 10.0.0.0/8.
