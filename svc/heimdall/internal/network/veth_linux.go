...
func hostVethIfindex(netnsPath string) (uint32, error) {
	...
	if err != nil {
		return 0, fmt.Errorf("open host netns: %w", err)
	}
	...
	if err := unix.Setns(int(podNs.Fd()), unix.CLONE_NEWNET); err != nil {
		return 0, fmt.Errorf("setns into pod: %w", err)
	}
	...
	if err := unix.Setns(int(origNs.Fd()), unix.CLONE_NEWNET); err != nil {
		...
		runtime.LockOSThread()
		return 0, fmt.Errorf("setns back to host: %w", err)
	}
...
func findVethPeerIfindex() (uint32, error) {
	...
	if err != nil {
		return 0, fmt.Errorf("netlink link list: %w", err)
	}
	...
	return 0, errors.New("no veth found in pod netns")
}
